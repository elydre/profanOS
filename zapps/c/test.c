...
// static int transport_read(void *ctx, unsigned char *buf, size_t len) {
//     return mlw_tcp_recv((mlw_tcp_t *)ctx, buf, len, -1);
// }

// static int transport_write(void *ctx, const unsigned char *buf, size_t len) {
//     return mlw_tcp_send((mlw_tcp_t *)ctx, (void *)buf, len);
// }

// size_t my_entropy(void *ctx, unsigned char *buf, size_t max_len) {
//     (void)ctx;
...
// /* Exemple principal : connecte TLS et récupère une page HTTP */
// int https_get_example(void *transport_ctx)
// {
...
//     /* --- Préparer les trust anchors si tu en as (optionnel) ---
//        Si tu veux valider les certificats, génère TAs.c/.h (brssl ta /
//        pycert_bearssl) et définis TAs / TAs_NUM. Sinon : NULL, 0. */
//     extern const br_x509_trust_anchor TAs[]; /* optionnel */

//     /* Initialiser le moteur X.509 minimal.
//        2e arg : hash utilisé pour comparer les DN (ici SHA-256). */
//     br_x509_minimal_init(&xc, &br_sha256_vtable,
//                          /*trust_anchors=*/ (TAs_NUM? TAs : NULL),
//                          /*trust_anchors_num=*/ (TAs_NUM? TAs_NUM : 0));
//     /* Initialise le profil client "full" (algos, suites, etc.) */
//     br_ssl_client_init_full(&sc, &xc,
//                             (TAs_NUM? TAs : NULL),
//                             (TAs_NUM? TAs_NUM : 0));
//     /* Buffer TLS (bidirectionnel) */
//     br_ssl_engine_set_buffer(&sc.eng, iobuf, sizeof iobuf, 1);

//     /* --- Injecter de l'entropie (obligatoire si pas d'OS RNG intégré) --- */
//     unsigned char seed[48]; /* 48 bytes ~ 384 bits ; >= 16 bytes suffisent souvent */
//     size_t have = my_entropy(NULL, seed, sizeof seed);
//     if (have == 0) {
//         fprintf(stderr, "Pas d'entropie disponible!\n");
//         return -1;
//     }
//     br_ssl_engine_inject_entropy(&sc.eng, seed, have);
//     memset(seed, 0, sizeof seed); /* effacer la seed après usage */

//     /* SNI / Hostname (utile pour la vérif. de cert) */
//     br_ssl_client_reset(&sc, "www.google.com", 0);

//     /* --- wrapper I/O: on connecte BearSSL à notre transport custom --- */
//     br_sslio_init(&ioc, &sc.eng,
//                   transport_read, transport_ctx,
//                   transport_write, transport_ctx);
...
// 		"Connection: close\r\n"
// 		"\r\n";
//     if (br_sslio_write(&ioc, (const unsigned char *)req, (int)strlen(req)) < 0) {
//         fprintf(stderr, "Erreur br_sslio_write\n");
//         return -1;
//     }
//     if (br_sslio_flush(&ioc) < 0) {
//         fprintf(stderr, "Erreur br_sslio_flush\n");
//         return -1;
//     }

//     /* Lecture de la réponse TLS (bloquant selon my_recv) */
//     unsigned char buf[1024];
//     for (;;) {
//         int r = br_sslio_read(&ioc, buf, sizeof buf);
//         if (r == 0) break;      /* fermé proprement */
//         if (r < 0) {
//             fprintf(stderr, "Erreur br_sslio_read : %d\n", r);
//             return -1;
//         }
...
//     uint32_t ip = mlw_ip_from_str("74.125.142.106");
//     mlw_tcp_t *inst = mlw_tcp_open(ip, 443);
//     if (!inst) {
//         printf("Failed to open MLW instance\n");
//         return 1;
//     }
//     https_get_example((void *)inst);
...
//     // if (mlw_tcp_send(inst, (void *)req, strlen(req))  <= 0) {
...
// 	// while (1) {
//     // 	char packet[500] = {0};
//     // 	int size = mlw_recv(inst, packet, 499, 1000);
//     // 	if (size <= 0) {
// 	// 		break;
//     // 	}
//     //     else {
