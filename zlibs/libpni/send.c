...
static void compute_checksum(uint8_t *buffer, size_t data_len) {
	...
	udp_header_t *udp_hdr = (udp_header_t *)(buffer + sizeof(ethernet_header_t) + sizeof(ip_header_t));

	uint16_t ip_hdr_data[sizeof(ip_header_t) / 2] ={0};
	uint32_t ip_sum = 0;
	memcpy(ip_hdr_data, ip_hdr, sizeof(ip_header_t));
...
int pni_send(uint16_t src_p, uint16_t dest_p, uint8_t *data, size_t data_len, uint8_t *dest_ip) {
	...
	memset(buffer, 0, sizeof(ethernet_header_t) + sizeof(ip_header_t) + sizeof(udp_header_t) + data_len);
