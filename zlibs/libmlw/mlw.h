...
int mlw_tcp_send_helper(uint16_t src_port,
				uint32_t dest_ip, uint32_t dest_port, uint32_t seq,
				uint32_t ack, uint8_t flags, uint8_t *data, int len);

int mlw_tcp_send(uint16_t src_port, uint32_t dest_ip, uint16_t dest_port, void *data, int len);
int mlw_tcp_recv(void **data, int *len, uint32_t dest_port, uint32_t src_ip, int timeout); // timeout = 0 -> inf, timeout < 0 -> no timeout
