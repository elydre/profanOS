...
ssize_t socket_udp_send(socket_t *sock, const uint8_t *buffer, size_t len, uint32_t dip, uint16_t dport, int flags) {
    udp_t *data = sock->data;
    if (len > 1450)
        return -EMSGSIZE;
    if (data->send_len == 64 && (flags & MSG_DONTWAIT))
        return -EAGAIN;
    while (data->send_len == 64)
...
ssize_t socket_udp_sendto(socket_t *sock, const void *buf, size_t len, int flags,
    ...
    if (!dest_addr && addrlen == 0)
        return socket_udp_send(sock, buf, len, 0, 0, 0);
    ...
    if (addr2->sin_port == 0)
        return -EINVAL;
    return socket_udp_send(sock, buf, len, addr2->sin_addr.s_addr, addr2->sin_port, flags);
}
