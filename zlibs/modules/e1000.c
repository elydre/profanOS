...
// array size is 100
static uint8_t ap[]  = {
  0xbb, 0x1d, 0x70, 0xba, 0xfd, 0x03, 0xec, 0x24, 0x20, 0x3c, 0x00, 0x74, 0xf6, 0xba, 0xf8, 0x03,
  0x8a, 0x07, 0xee, 0x43, 0x80, 0x3f, 0x00, 0x74, 0x02, 0xeb, 0xe8, 0xeb, 0x18, 0x73, 0x61, 0x6c,
  0x75, 0x74, 0x20, 0x64, 0x65, 0x70, 0x75, 0x69, 0x73, 0x20, 0x6c, 0x65, 0x20, 0x63, 0x6f, 0x65,
  0x75, 0x72, 0x31, 0x10, 0x00, 0xe4, 0x64, 0xa8, 0x02, 0x75, 0xfa, 0xb0, 0xed, 0xe6, 0x60, 0xe4,
  0x64, 0xa8, 0x01, 0x74, 0xfa, 0xe4, 0x60, 0x3c, 0xfa, 0x75, 0xfe, 0xe4, 0x64, 0xa8, 0x02, 0x75,
  0xfa, 0xb0, 0x04, 0xe6, 0x60, 0xe4, 0x64, 0xa8, 0x01, 0x74, 0xfa, 0xe4, 0x60, 0x3c, 0xfa, 0x75,
  0xfe, 0xf4, 0xeb, 0xfd
};
...
void parse_madt(SDT_header_t *header)
{
	...
	uint32_t self_apic_id = get_apic_id();
	kprintf("BOOT CPU has apid_id %d\n", self_apic_id);
...
void foreach_entry(SDT_header_t *header, void (*func)(SDT_header_t *), int ptr_size) {
	...
	uint32_t entries_len = (header->length - sizeof(SDT_header_t)) / ptr_size;
	kprintf("found %d entries\n", entries_len);

	for (uint32_t i = 0; i < entries_len; i++) {
		uint32_t addr = ((uint32_t)&header[1]) + i * ptr_size;
...
static int test() {
	...
	kprintf("found rsdp table at address %p version %d\n", rsdp, rsdp->revision);

	if (rsdp->revision < 2) {
		foreach_entry((void *)rsdp->rsdt_addr, treat_entry, 4);
		return 1;
	}
	else {
		XSDP_t *xsdp = (void *)rsdp;
		kprintf("xsdt %x%x\n", (uint32_t)(xsdp->xsdt_addr >> 32), (uint32_t)xsdp->xsdt_addr);
		foreach_entry((void *)(uint32_t)xsdp->xsdt_addr, treat_entry, 8);
	}
